...
import Deriving.Aeson.Stock (Snake, PrefixedSnake, CustomJSON (..))
import Text.Read (readMaybe) 
import Control.Applicative ((<|>))
import qualified Data.Text as T
import Control.Lens ((&), (%~), ix)
import Data.Char (toUpper, toLower)
import Options.Applicative (strOption, switch, long, metavar, help, short, info, helper, (<**>), fullDesc, progDesc, header, showDefault, value, ParserInfo, option, auto, optional, str, showDefaultWith, flag')
...
data SelectedBackend
  ...
  | File FileBackendConfig -- ^ Use a flat file as the backend
  | Postgres PostgresBackendConfig -- ^ Use a PostgreSQL database as the backend
  deriving (Eq, Show, Read)
...
httpConfigParser =
  ...
  where
    maxUploadParser = optional . option auto $
...
data Configuration = Configuration
  { http :: HttpConfig
  , purgeDiffsOnCheckpoint :: Bool -- ^ Whether or not to purge all "diff" records when storing a new checkpoint
  , purgeCheckpointsOnCheckpoint :: Bool -- ^ Whether or not to purge all "checkpoint" records when storing a new checkpoint
...
instance FromJSON Configuration where
  parseJSON = withObject "Configuration" $ \o ->
    Configuration
      ...
      <*> (fromMaybe (purgeCheckpointsOnCheckpoint def) <$> o .:? "purge_checkpoints_on_checkpoint")
...
instance Semigroup Configuration where
  x <> y =
    let applyIfNotDef :: forall a
                       . Eq a => (Configuration -> a)
    ...
    in  applyIfNotDef http (\a b -> b { http = a })
          ...
          . applyIfNotDef purgeCheckpointsOnCheckpoint (\a b -> b { purgeCheckpointsOnCheckpoint = a })
...
configParser =
  ...
  where
    ...
    checkpointEveryOffsetParser = option durationUnitOffset $
      long "checkpoint-interval-offset"
        ...
        <> help "Offset within interval - 's' is \"seconds\", 'm' is \"minutes\", _h_our, _d_ay, _w_eek"
      where
