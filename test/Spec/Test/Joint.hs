...
import Lib.Types.Permission (CollectionPermission (Delete, Read, Create), CollectionPermissionWithExemption (..))
import Lib.Actions.Safe.Store (storeEntity, storeActor, storeGroup, storeSpace, addMember)
import Lib.Actions.Safe.Update (updateEntitySpace)
import Lib.Actions.Safe.Update.Group (setEntityPermission, setUniversePermission, setMemberPermission)
...
jointTests :: Spec
jointTests = describe "Joint" $
  it "Move Request Between Public Spaces" $
    forAll arbitraryEmptyShared $ \(s,adminActor,adminGroup) ->
      property $ \(actorA :: ActorId
      , groupA :: GroupId, actorB :: ActorId, groupB :: GroupId, spaceA :: SpaceId, spaceB :: SpaceId, eId :: EntityId, vId :: VersionId) -> do
        let s' = flip execState s $ do
              worked <- storeActor (NE.singleton adminActor) actorA
              unless worked $
                error $ "Couldn't store actor " <> show actorA
              worked <- storeActor (NE.singleton adminActor) actorB 
              unless worked $
                error $ "Couldn't store actor " <> show actorB
              worked <- storeGroup (NE.singleton adminActor) groupA
              unless worked $
                error $ "Couldn't store group " <> show groupA
              worked <- storeGroup (NE.singleton adminActor) groupB 
              unless worked $
                error $ "Couldn't store group " <> show groupB

              worked <- setMemberPermission (NE.singleton adminActor) Create adminGroup groupA
              unless worked $
                error $ "Couldn't set member permission " <> show groupA
              worked <- setMemberPermission (NE.singleton adminActor) Create adminGroup groupB
              unless worked $
                error $ "Couldn't set member permission " <> show groupB

              worked <- addMember (NE.singleton adminActor) groupA actorA
              unless worked $
                error $ "Couldn't add member " <> show (groupA, actorA)
              worked <- addMember (NE.singleton adminActor) groupB actorB
              unless worked $
                error $ "Couldn't add member " <> show (groupB, actorB)

              worked <- storeSpace (NE.singleton adminActor) spaceA
              unless worked $
                error $ "Couldn't store space " <> show spaceA
              worked <- storeSpace (NE.singleton adminActor) spaceB 
              unless worked $
                error $ "Couldn't store space " <> show spaceB

              worked <- setUniversePermission (NE.singleton adminActor)
                (CollectionPermissionWithExemption Read False) groupA 
              unless worked $
                error $ "Couldn't set universe permission " <> show groupA
              worked <- setUniversePermission (NE.singleton adminActor)
                (CollectionPermissionWithExemption Read False) groupB 
              unless worked $
                error $ "Couldn't set universe permission " <> show groupB

              worked <- setEntityPermission (NE.singleton adminActor) Delete groupA spaceA
              unless worked $
                error $ "Couldn't set space permission " <> show spaceA
              worked <- setEntityPermission (NE.singleton adminActor) Delete groupB spaceB
              unless worked $
                error $ "Couldn't set space permission " <> show spaceB
              ...
              unless worked $
                error $ "Couldn't move entity " <> show eId
