...
import Spec.Sample.Store (SampleStore, storeSample, arbitraryEmptyShared, arbitraryShared)
    ...
    describe "Should Succeed" $ do
      it "Move An Entity to different Space" $
        forAll arbitraryEmptyShared $ \(s, adminActor, adminGroup) ->
          property $ \(aId :: ActorId, gId :: GroupId, sId :: SpaceId, sId' :: SpaceId, eId :: EntityId, vId :: VersionId) -> do
            ...
            (s' ^. store . toSpaces . at sId . non mempty . entities . at eId) `shouldBe` Nothing
            (s' ^. store . toSpaces . at sId' . non mempty . entities . at eId) `shouldBe` Just ()
        ...
        in  forAll gen $ \(s, adminActor, adminGroup) ->
              ...
              in  forAll selectVersionAndEntity $ \(eIdOfVId, vId) ->
                    forAll (elements . HM.keys $ HM.delete eIdOfVId entities) $ \subId -> do
                      ...
                      (s' ^? store . toVersions . ix vId . subscriptions . ix subId) `shouldBe` Just ()
        ...
        in  forAll gen $ \(s, adminActor, adminGroup) ->
              ...
              in  forAll (elements $ HM.keys subs) $ \subId ->
                    forAll (elements . HS.toList . fromJust $ HM.lookup subId subs) $ \vId -> do
                      ...
                      (s' ^? store . toVersions . ix vId . subscriptions . ix subId) `shouldBe` Nothing
                      (s' ^? temp . toSubscriptionsFrom . ix subId . ix vId) `shouldBe` Nothing
      it "Remove a Version" $
        let gen = suchThat arbitraryShared $ \(s,_,_) ->
             not . null . HM.filter (\e -> length (e ^. versions) > 1) $ s ^. store . toEntities
        in  forAll gen $ \(s, adminActor, adminGroup) ->
              let entsWith2OrMoreVersions = HM.filter (\e -> length (e ^. versions) > 1)
                                          $ s ^. store . toEntities
              in  forAll (elements $ HM.toList entsWith2OrMoreVersions) $ \(eId, e) ->
                    forAll (elements . NE.toList $ e ^. versions) $ \vId -> do
                      ...
                      (filter (== vId) . fromMaybe mempty . fmap NE.toList
                        $ s' ^? store . toEntities . ix eId . versions) `shouldBe` mempty
      ...
      it "Re-Fork an Entity" $
        let gen = do
              let genSharedAndEnt = do
                    (shared, f, aA, aG) <- suchThatMap arbitrary $ \(xs, adminActor, adminGroup) ->
                      ...
                      in  if null forking || length (s ^. store . toVersions) < 5
                          then Nothing else Just (s, forking, adminActor, adminGroup)
              ...
              (s, eId, notForked, adminA, adminG) <- suchThatMap genSharedAndEnt $ \(shared, eId, e, aA, aG) ->
                let notForked = filter (\vId -> e ^. fork /= Just vId) . HM.keys $ shared ^. store . toVersions
  ...
  describe "Group" $ do
    ...
    describe "Should Succeed" $ do
      it "deleting parent relationship should affect both nodes, and unrelate them" $
        let gen = suchThat arbitraryShared $ \(s,_,_) ->
              not . null . HM.filter (\g -> isJust (g ^. prev)) $ s ^. store . toGroups . nodes
        in  forAll gen $ \(s, adminActor, adminGroup) ->
              ...
              in  forAll (elements (HM.toList gsWithParent)) $ \(gId :: GroupId, g) -> do
                    ...
                    (s' ^? store . toGroups . nodes . ix parentId . next . ix gId) `shouldBe` Nothing
