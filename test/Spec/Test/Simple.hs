...
simpleTests = do
  ...
  describe "CollectionPermissionWithExemption" $ do
    it "has a lower bound" $
      property $ \(x :: CollectionPermissionWithExemption) -> x `shouldSatisfy` (>= minBound)
    it "has an upper bound" $
      property $ \(x :: CollectionPermissionWithExemption) -> x `shouldSatisfy` (<= maxBound)
    it "is a semigroup" $
      property $ \(x :: CollectionPermissionWithExemption) y z -> (x <> y) <> z `shouldBe` x <> (y <> z)
    it "is a monoid left" $
      property $ \(x :: CollectionPermissionWithExemption) -> x <> mempty `shouldBe` x
    it "is a monoid right" $
      property $ \(x :: CollectionPermissionWithExemption) -> mempty <> x `shouldBe` x
    it "is commutative" $
      property $ \(x :: CollectionPermissionWithExemption) y -> y <> x `shouldBe` x <> y
  describe "SinglePermission" $ do
    it "has a lower bound" $
      property $ \(x :: SinglePermission) -> x `shouldSatisfy` (>= minBound)
    it "has an upper bound" $
      property $ \(x :: SinglePermission) -> x `shouldSatisfy` (<= maxBound)
    it "is a semigroup" $
      property $ \(x :: SinglePermission) y z -> (x <> y) <> z `shouldBe` x <> (y <> z)
    it "is commutative" $
      property $ \(x :: SinglePermission) y -> y <> x `shouldBe` x <> y
